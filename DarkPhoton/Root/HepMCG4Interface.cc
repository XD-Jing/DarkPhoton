  ...
  : hepmcEvent(0)
{
    ...
    tr = new TTree("MCParticle","MCParticle");

    tr->Branch("EventID"  ,      &EventID   ,   "EventID/I");
    tr->Branch("dp_PDG"   ,      &dp_PDG    ,   "dp_PDG/I");
    tr->Branch("dp_Energy",      &dp_Energy ,   "dp_Energy/D");
    tr->Branch("dp_Momentum",    &dp_Momentum , "dp_Momentum[3]/D");
    tr->Branch("e_PDG"    ,      &e_PDG     ,   "e_PDG/I");
    tr->Branch("e_Energy" ,      &e_Energy  ,   "e_Energy/D");
    tr->Branch("e_Momentum",     &e_Momentum ,  "e_Momentum[3]/D");
                                ...
                                G4Event* g4event)
{
    ...
    for(HepMC::GenEvent::vertex_const_iterator vitr= hepmcevt->vertices_begin();
        ...
        for (HepMC::GenVertex::particle_iterator
            ...
            if(pdgcode == 2200) {
                dp_Energy   = pos.e();
                dp_Momentum[0] = pos.px();
                dp_Momentum[1] = pos.py();
                dp_Momentum[2] = pos.pz();
            }
            if(pdgcode == 11) {
                e_Energy    = pos.e();
                e_Momentum[0] = pos.px();
